      ...
      >
        {slides.map((slide, index) => (
          <SwiperSlide key={slide.id}>
            <div
              className="relative w-full h-[320px] sm:h-[420px] md:h-[520px] lg:h-[640px] overflow-hidden bg-cover bg-center transition-all duration-700 ease-in-out"
              style={{
            ...
            >
              ...
              <div className="relative mx-auto flex h-full max-w-7xl items-center px-6 sm:px-8 md:px-10 lg:px-12 animate__animated animate__fadeInUp animate__faster justify-start">
                <div className="flex w-full max-w-2xl flex-col gap-3 md:gap-4 text-left">
                  <span className="md:mx-0 w-fit rounded-full border border-white/20 bg-white/10 px-4 py-1 text-[10px] sm:text-xs font-semibold uppercase tracking-[0.24em] text-emerald-200 backdrop-blur-sm">
                    {slide.subtitle}
                  </span>

                  <h2 className="max-w-xl text-2xl sm:text-3xl md:text-5xl lg:text-6xl font-black leading-tight text-white">
                    {slide.title}
                  </h2>
